...
Homepage's Semaphore tile shows the last run's status at a glance (✅ OK /
❌ FAILED / ⏳ Running) plus how long ago it ran, via a `customapi` widget
hitting `GET /api/project/1/templates/8` with a Semaphore API bearer token
(`semaphore_api_token` in vault, generated once via `POST /api/user/tokens`
— it doesn't expire and isn't regenerated by any playbook, so if it's ever
revoked in the Semaphore UI the widget silently stops updating and needs a
new one minted and pasted into vault.yml by hand).
